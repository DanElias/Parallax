...
//Esta función abre el modal de más información con ajax cuando el usuario da click en mas informacion en la tabla de cuentas
    function mostrar_informacion_cuenta(id_cuenta){
     ...
     //Esta función abre el modal con el form de editar evento cuando el usuario da click en editar en la tabla de cuentas
    ...
    //Esta función se manda llamar con onkeyup() desde el form de editar cuenta y el form de registrar cuenta
    ...
    function validar_campo_cuenta(id_input,id_respuesta){ //Aqui le envio el id de donde aparecerá la respuesta, ese mismo id es con el que llamare la funcion
        var input=document.getElementById(id_input).value; //guardo lo que el usuario está escribiendo
        var json;
        
        $.post('_validaciones_cuenta.php', { id_respuesta : id_respuesta, input : input} )
        .done(function(data){ //Descubrí que html(data) literalmente inserta un archivo json en el html
            ...
            if($('#validar_nombre_cuenta').text()=="" && $('#validar_descripcion_cuenta').text()==""){
                $('#submit').show();
            }else{
                 $('#submit').hide();
            }
            
            if($('#evalidar_nombre_cuenta').text()=="" && $('#evalidar_descripcion_cuenta').text()==""){
                $('#esubmit').show();
            }else{
                 $('#esubmit').hide();
            }
